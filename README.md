...
Set your Anthropic API key as an environment variable — never paste it into the source, so it can't be committed by accident:
...
The script reads the key from the environment at runtime. If you keep a local `.env` file, make sure it's listed in `.gitignore`.
